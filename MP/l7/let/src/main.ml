...
let rec eval (e : expr) : value =
  ...
  | Sum (x, e1, e2, e3) ->
    let start_val = match eval e1 with
    | VInt n -> n
    | _ -> failwith "type error" in
    let end_val = match eval e2 with
    | VInt n -> n
    | _ -> failwith "type error" in
    let rec aux i acc = 
...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | Sum (x, e1, e2, e3) ->
    let start_val = match eval_env env e1 with
    | VInt n -> n
    | _ -> failwith "type error" in
    let end_val = match eval_env env e2 with
    | VInt n -> n
    | _ -> failwith "type error" in
    let rec aux i acc = 
      if i > end_val then acc
      else 
        let new_env = M.add x (VInt i) env in
        let res = match eval_env new_env e3 with
        | VInt n -> n
        | _ -> failwith "type error" in
      aux (i + 1) (acc + res) in
    VInt (aux start_val 0)
